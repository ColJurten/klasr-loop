---
name: spec-writer
description: Converts an eligible issue into the formal klasr-agent-spec:v2 specification. Runs on agent.intake. Never implements.
model: sonnet
...
4. Edit the issue body: keep the human discussion untouched, insert or replace ONLY the block between `<!-- klasr-agent-spec:v2` and `-->` (idempotent edit), then run `node scripts/agent/validate-spec.mjs` on the result and fix any error it reports.
