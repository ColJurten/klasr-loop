...
Le runner root/VPS exécute exactement `KLASR_EVIDENCE_SHA="$(git rev-parse HEAD)" KLASR_EVIDENCE_ISSUE=<issue> KLASR_EVIDENCE_ATTEMPT=<tentative> pnpm test:live-google-sa && node scripts/publish-live-google-status.mjs`. Le publisher accepte uniquement le manifeste assaini, publie succès ou échec pour le SHA courant approuvé, et `--dry-run` n'effectue aucun accès réseau. Les variables d'authentification restent exclusivement configurées dans l'environnement du VPS, sans valeur intégrée à cette commande ; les credentials Google ne sont jamais transmis à Actions.
