...
describe('DashboardController', () => {
  ...
  it('renders production dashboard data without eagerly listing the full Drive', async () => {
    ...
    await expect(controller.get('org_1')).resolves.toEqual(expect.objectContaining({ mode: 'production', inputItems: [] }));
    expect(sync.listInputItems).not.toHaveBeenCalled();
  });
  ...
  it('retains deterministic input listing in local mode', async () => {
    ...
    await expect(controller.get('org_1')).resolves.toEqual(expect.objectContaining({ mode: 'local', inputItems }));
    expect(sync.listInputItems).toHaveBeenCalledWith('org_1');
  });
});
